...
function scan(requests, head, direction, maxTrack) {
  ...
  if (direction === "right") {
    // Add max track if not already in requests
    if (tr.indexOf(maxTrack) === -1) {
      tr.push(maxTrack);
    }
    ...
    // Then process requests to the left
    for (let i = startIndex - 1; i >= 0; i--) {
      requestOrder.push(tr[i]);
    }
  } else if (direction === "left") {
    // Add track 0 if not already in requests
    if (tr.indexOf(0) === -1) {
      tr.push(0);
    }
    ...
    // Then process requests to the right
    for (let i = startIndex + 1; i < tr.length; i++) {
      requestOrder.push(tr[i]);
    }
  }
...
function execute() {
    ...
    if (allOk) {
        ...
        function displaySeekOp(){
            ...
            dIcon.id = 'url'; dIcon.download = "SCAN.jpeg";
